...
class User(Base):
    ...
    histories = relationship(
        "QuotationHistory", back_populates="user", cascade="all, delete-orphan"
    )
    favorites = relationship(
        "QuotationFavorite", back_populates="user", cascade="all, delete-orphan"
    )
...
class QuotationHistory(Base):
    ...
    favorites = relationship(
        "QuotationFavorite", back_populates="history", cascade="all, delete-orphan"
    )

    # 复合索引优化查询性能
    __table_args__ = (
        Index("ix_user_computed_at", "user_id", "computed_at"),
        Index("ix_worker_type_computed_at", "worker_type", "computed_at"),
    )
...
class QuotationFavorite(Base):
    ...
    history_id = Column(
        Integer,
        ForeignKey("quotation_history.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
