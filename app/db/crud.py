...
from app.db.models import User, AppSettings, WorkerProfile, QuotationHistory, QuotationFavorite
...
def update_app_settings(db: Session, **kwargs) -> AppSettings:
...
def upsert_worker_profile(db: Session, name: str, monthly_salary: float, machines_operated: int) -> WorkerProfile:
    ...
    if profile:
        ...
        profile = WorkerProfile(name=name, monthly_salary=monthly_salary, machines_operated=machines_operated)
        db.add(profile)
...
def create_history(db: Session, user_id: int, request_payload: dict, result_payload: dict,
                   worker_type: str, unit_price: float, total_price: float) -> QuotationHistory:
...
def get_user_histories(db: Session, user_id: int, offset: int = 0, limit: int = 20) -> List[QuotationHistory]:
    return db.query(QuotationHistory)\
...
def get_history_by_id(db: Session, history_id: int, user_id: int) -> Optional[QuotationHistory]:
    return db.query(QuotationHistory)\
...
def create_favorite(db: Session, user_id: int, history_id: int, name: Optional[str] = None) -> QuotationFavorite:
...
def get_user_favorites(db: Session, user_id: int) -> List[QuotationFavorite]:
    return db.query(QuotationFavorite)\
        .filter(QuotationFavorite.user_id == user_id)\
        .order_by(QuotationFavorite.created_at.desc())\
        .all()


def get_favorite_by_id(db: Session, favorite_id: int, user_id: int) -> Optional[QuotationFavorite]:
    return db.query(QuotationFavorite)\
        .filter(QuotationFavorite.id == favorite_id, QuotationFavorite.user_id == user_id)\
        .first()
...
def check_favorite_exists(db: Session, user_id: int, history_id: int) -> bool:
    return db.query(QuotationFavorite)\
        .filter(QuotationFavorite.user_id == user_id, QuotationFavorite.history_id == history_id)\
        .first() is not None
